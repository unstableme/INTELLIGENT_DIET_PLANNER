...
def generate_recommendation(request):
    if request.method == 'POST':
        ...
        # Load the data from your Excel file using pandas
        df = pd.read_excel('C:\\Users\\user\Documents\FROMNARENDRA\idp\IntelligentDP\\foodData\project_data2.xlsx')
        ...
        for meal_type in meal_order:
            ...
            meal_items_with_type = [(meal_item, food_types[meal_item]) for meal_item in meal_items]
            meal_plan.append(meal_items_with_type)

            meal_frequency -= 1

            # Update the remaining calories
            remaining_calories -= sum([calories_info[meal_item] for meal_item in meal_items])
        ...
        for meal_items in meal_plan:
            ...
            # Create a list with the meal details
            meal_details = []
            for meal_item in meal_items:
                try:
                    food = Food_Details.objects.get(foodName=meal_item[0])
                    meal_detail = {
                        'foodName': food.foodName,
                        'calories': calories_info[meal_item[0]],
                        'rating': avg_rating,
                        'carbohydrates': food.carbohydrate_g,
                        'protein': food.protein_g,
                        'fat': food.fat_g,
            ...
            suggested_meals.append({
                'meal': [meal_item[0] for meal_item in meal_items],
                'calorie': sum([calories_info[meal_item[0]] for meal_item in meal_items]),
                'foods': meal_details
            })

        # Calculate the total calorie count for the meal plan
        total_calories = sum([sum([calories_info[meal_item[0]] for meal_item in meal_items]) for meal_items in meal_plan])
